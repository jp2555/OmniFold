...
parser.add_argument('--weights', default='/pscratch/sd/j/jing/H1PCT/weights_saved/weights_mom_300e6', help='Folder to store trained weights')
...
# else:
mc_info['data'] = MCInfo('Data1516',flags.N,flags.data_folder,config,flags.cf,flags.q2_int,is_reco=True,use_fiducial_mask=True)   
#Loading weights from training
for mc_name in mc_names:
    print("{}.h5".format(mc_name))    
    mc_info[mc_name] = MCInfo(mc_name,flags.N,flags.data_folder,config,flags.cf,flags.q2_int,is_reco=flags.plot_reco,use_fiducial_mask=True)
        ...
        if flags.comp:
            ...
            if flags.sys == True: #load systematic variations
                for sys in opt.sys_sources:
                    ...
                    mc_info[sys] = MCInfo(mc_name.replace("Pythia_nominal",sys),int(flags.N),flags.data_folder,config,flags.cf,flags.q2_int,is_reco=flags.plot_reco,use_fiducial_mask=True)
                    ...
                    sys_variations[sys] = mc_info[sys].LoadTrainedWeights( # first evaluate weights, here define a clip mask
...
for var in gen_var_names:
    ...
    if flags.plot_reco:
        plot_folder+='_reco'
    plot_folder+='_{}'.format(flags.nom,)
    ...
    if flags.q2_int>0:
        ...
        if flags.sys:
            ...
            fig_sys.savefig(os.path.join(plot_folder,"sys_{}_{}_{}_{}_nom{}.{}".format(var,flags.niter,flags.q2_int,flags.clip,flags.nom,flags.img_fmt)))
    else:
