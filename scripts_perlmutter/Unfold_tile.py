...
# sys_translate = {
#     'Pythia_OFF': "Pythia_sys_0",'Pythia_UP': "Pythia_sys_1",'Pythia_DOWN': 'Pythia_sys_2', # PURW
#     'Pythia_CORE': 'Pythia_sys_3','Pythia_EDGE': 'Pythia_sys_4', # TILE
#     'Sherpa_Lund': 'Pythia_sys_5','Sherpa_AHADIC': 'Pythia_sys_6', # Modeling
#     'Pythia_TRACK1': 'Pythia_sys_7','Pythia_TRACK2': 'Pythia_sys_8','Pythia_TRACK3': 'Pythia_sys_9','Pythia_TRACK4': 'Pythia_sys_10',
#     'Pythia_JES45': 'Pythia_sys_11','Pythia_JES46': 'Pythia_sys_12','Pythia_JES47': 'Pythia_sys_13','Pythia_JES48': 'Pythia_sys_14',
#     }

# if flags.closure:
#     mc_names = opt['MC_NAMES']
# else:
#     mc_names = [ sys_translate[ orig_name ] for orig_name in opt['MC_NAMES'] ]
mc_names = opt['MC_NAMES']
...
data_name = 'Data1516_CORE'
if flags.closure:
    data_name = 'Sherpa_Lund' 
    #data_name = 'Pythia_nominal'
...
for mc_name in mc_names:
    ...
    if flags.closure:
        ...
        print('!!!!!pseudo-data counts: ', weights_data.shape[0])
#        input()

    else:
        #ntest = int(5e6)
        data_vars = np.concatenate([np.expand_dims(data[var][hvd.rank()::hvd.size()],-1) for var in var_names],-1)
        weights_data = data['wgt'][hvd.rank()::hvd.size()]
        print('!!!!!data counts: ', weights_data.shape[0])
        #input()
        if flags.mode != 'standard':
    ...
    if flags.mode == 'standard':            
        ...
        #data_vars = np.log(data_vars)
        mc_reco[:,0][pass_reco==0] = -10
        mc_reco[:,0][pass_reco==-1] = -10
        mc_gen[:,0][pass_truth==0] = -10
        mc_gen[:,0][pass_truth==-1] = -10
    ...
    print('!!!!!mc counts:', weights_MC_sim.shape[0])
#    input()
