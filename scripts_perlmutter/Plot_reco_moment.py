...
parser.add_argument('--fast', action='store_true', default=False,help='Debugging with one bin')
parser.add_argument('--plot_reco', action='store_true', default=True,help='Plot reco level comparison between data and MC predictions')
    ...
    for order in orders: # one list / order
        ...
        for q2_int in range(1,len(xaxis)+1):
            ...
            if flags.qg == True:
                ...
                value = 1 if mom_cf[pT][flavor]['nominal'][order] > 1 else mom_cf[pT][flavor]['nominal'][order]
                py_value = 1 if py_mom_cf[pT][flavor]['nominal'][order] > 1 else py_mom_cf[pT][flavor]['nominal'][order]

                data_pred.append( value )
                py_pred.append( py_value )
