...
parser.add_argument('-N',type=float,default=15e6, help='Number of events to evaluate')
...
# standalone_predictions = ['Herwig','Pythia']
...
# store the array to plot pT ratios
var_dict, mc_var_dict = {}, {} 
...
for var in gen_var_names:
    ...
    if flags.plot_reco:
        ...
        if flags.plot_ratio:
            ...
            if var == 'gen_trkpt_c':
                # reset the binning for the pT ratio
                binning=opt.dedicated_binning['gen_tf_c']
                xaxis = 0.5*(binning[:-1] + binning[1:])
                trkpt_pred,_=np.histogram(data_var,weights=weight_data*mc_info[data_name].nominal_wgts)
                pt_pred,_=np.histogram(var_dict['gen_pt_c'],weights=weight_data*mc_info[data_name].nominal_wgts)
                print(trkpt_pred.shape, pt_pred.shape)
                chargedFrac = np.divide(trkpt_pred, pt_pred)
                print(chargedFrac)
                data_pred,_=np.histogram(chargedFrac,weights=weight_data*mc_info[data_name].nominal_wgts,bins=binning,density=True)
            elif var == 'gen_trkpt_f':
                # same, reset the binning for the pT ratio
                binning=opt.dedicated_binning['gen_tf_c']
                xaxis = 0.5*(binning[:-1] + binning[1:])
                trkpt_pred,_=np.histogram(data_var,weights=weight_data*mc_info[data_name].nominal_wgts)
                pt_pred,_=np.histogram(var_dict['gen_pt_c'],weights=weight_data*mc_info[data_name].nominal_wgts)
        ...
        if flags.plot_reco:
            ...
            if flags.plot_ratio:
                if var == 'gen_trkpt_c':
                    ...
                    pt_pred,_=np.histogram(var_dict['gen_pt_c'],weights=weight_data*mc_info[data_name].nominal_wgts)
                    chargedFrac = np.divide(trkpt_pred, pt_pred)
                    data_pred,_,_=ax0.hist(chargedFrac,weights=weight_data*mc_info[data_name].nominal_wgts,bins=binning,label=label_data,density=True,color="black",histtype="step")
                elif var == 'gen_trkpt_f':
                    trkpt_pred,_=np.histogram(data_var,weights=weight_data*mc_info[data_name].nominal_wgts)
                    pt_pred,_=np.histogram(var_dict['gen_pt_c'],weights=weight_data*mc_info[data_name].nominal_wgts)
                    chargedFrac = np.divide(trkpt_pred, pt_pred)
    ...
    for i,mc_name in enumerate(mc_names):
        ...
        if var in mc_var_dict:
            mc_var_dict[var].append(mc_var)
        else:
            mc_var_dict[var] = [mc_var] 
        print(mc_var_dict[var])

        if flags.plot_ratio:
            if var == 'gen_trkpt_c':
                mc_chargedFrac = np.divide(mc_var_dict['gen_trkpt_c'][i], mc_var_dict['gen_pt_c'][i] )#,where=(mc_var_dict['gen_pt_c'] != 0))
                # print(mc_var_dict['gen_trkpt_c'][i].shape, mc_var_dict['gen_pt_c'][i].shape)
                # print(mc_chargedFrac.shape, mc_info[mc_name].nominal_wgts.shape)
                pred,_=np.histogram(mc_chargedFrac,weights=mc_info[mc_name].nominal_wgts,bins=binning,density=True)
            elif var == 'gen_trkpt_f':
                mc_chargedFrac = np.divide(mc_var_dict['gen_trkpt_f'][i], mc_var_dict['gen_pt_f'][i] )#,where=(mc_var_dict['gen_pt_f'] != 0))
                pred,_=np.histogram(mc_chargedFrac,weights=mc_info[mc_name].nominal_wgts,bins=binning,density=True)
            else:
