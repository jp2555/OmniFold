...
def PlotUnc(xaxis,var,values,xlabel='',add_text=''):
    ...
    for sys in values:
        ...
        if sys == 'ensem':
            ...
            plt.step(xaxis, values[sys],where='mid',color=opt.sys_sources[sys],label=sys)#opt.sys_translate[sys])
    ...
    if sys == 'stat':
        plt.ylim([0,1])
    # else:
    #     plt.ylim([1.5*min_ylim,1.5*max_ylim])
    return fig
        ...
        if flags.comp:
            ...
            if flags.sys == True: #load systematic variations
                for sys in opt.sys_sources:
                    ...
                    print(mc_name.replace("Pythia_nominal",sys))
