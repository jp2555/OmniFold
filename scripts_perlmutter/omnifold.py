...
class Multifold():
    def __init__(self,nevts,nstrap=0,ntrial=1,mode='hybrid',version = 'Closure',config_file='config_omnifold.json',verbose=1):
        self.mode = mode
        ...
        if self.ntrial>1:
            self.weights_folder = '../weights_trial'
    ...
    def RunStep2(self,i):
        ...
        self.RunModel(
            ...
            #np.concatenate((np.ones(self.weights_mc.shape), self.weights_mc*self.weights_pull)),
    ...
    def RunModel(self,sample,labels,weights,iteration,model,stepn,global_vars=None):
        ...
        if hvd.rank() ==0:
            if self.nstrap>0:                
                ...
                callbacks.append(
                    ModelCheckpoint('{}/{}_{}_iter{}_step{}_trial{}.h5'.format(self.weights_folder,base_name,self.version,iteration,stepn,self.ntrial),
                                    save_best_only=True,mode='auto',period=1,save_weights_only=True))
            else:
                callbacks.append(
